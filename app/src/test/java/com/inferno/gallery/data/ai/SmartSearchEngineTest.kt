...
class SmartSearchEngineTest {
    ...
    fun testBPETokenizerMapping() {
        ...
        val whatsappQuery = tokenizer.encode("screenshot of whatsapp chat")
        val expectedPrefix = longArrayOf(49406L, 12646L, 539L, 10119L, 2402L, 49407L)
        assertArrayEquals(
            "Tokenizer should match CLIP BPE IDs for multi-subword app names",
            expectedPrefix,
            whatsappQuery.take(expectedPrefix.size).toLongArray()
        )
    }
